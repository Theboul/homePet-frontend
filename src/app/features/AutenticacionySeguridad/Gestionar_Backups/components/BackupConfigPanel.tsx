...
const getRetentionDays = (cfg?: BackupConfig): number =>
  (cfg as any)?.['dias_retención'] ?? (cfg as any)?.dias_retenciÃ³n ?? 30;

const getLastBackup = (cfg?: BackupConfig): string | null =>
  (cfg as any)?.['último_backup'] ?? (cfg as any)?.Ãºltimo_backup ?? null;

const getNextBackup = (cfg?: BackupConfig): string | null =>
  (cfg as any)?.['próximo_backup_programado'] ?? (cfg as any)?.prÃ³ximo_backup_programado ?? null;
